...
void DateTimeSelector::setupUi()
{
	...
	m_YYYYMMDDWidget = new QWidget(this);
...
void DateTimeSelector::setDate(const QDate& date)
{
...
void DateTimeSelector::setTime(const QTime& time)
{
...
void DateTimeSelector::setDateTime(const QDateTime& dateTime)
{
	setDate(dateTime.date());
	setTime(dateTime.time());
}

void DateTimeSelector::edit(FadeEffectButton* button, const QValidator* validator)
{
	// 字体清单
	QFont timeFont = QFont("NeverMind", 11, QFont::Normal);

	m_lineEdit->move(button->mapTo(this, QPoint(0, 0)));
